...
from .models import Articulo, CanalCliente, GrupoArticulo, ItemOrdenCompraCliente, LineaArticulo, ListaPrecio, Cliente, OrdenCompraCliente, TipoIdentificacion, Vendedor, EstadoOrden
...
def articulo_detail(request, articulo_id):
    ...
    # Guardar en el historial de productos visitadoss
...
#VISTAS PARA A EL CARRITO DE COMPRAS
@require_POST
...
def checkout(request):
    ...
    try:
        ...
        return render(request, 'myapp/cart/checkout.html', {
            'cart': cart,
            'cliente': cliente
        })
...
def order_detail(request, pedido_id):
    ...
    return render(request, 'myapp/cart/order_detail.html', {'orden': orden})
...
def send_order_confirmation_email(orden):
 ...
 html_content = render_to_string('emails/order_confirmation.html', {'orden': orden,
...
@login_required
def cancelar_orden(request, pedido_id):
    orden = get_object_or_404(OrdenCompraCliente, pedido_id=pedido_id)

    # Verifica si el usuario tiene permiso para cancelar la orden
    if orden.creado_por != request.user:
    ...
    if orden.estado != EstadoOrden.PENDIENTE:
        messages.warning(request, "Solo se pueden cancelar órdenes pendientes.")
        return redirect('order_detail', pedido_id=pedido_id)

    try:
        orden.estado = EstadoOrden.CANCELADA
        orden.save()
        messages.success(request, f"La orden {orden.nro_pedido} fue cancelada exitosamente.")
    except Exception as e:
        messages.error(request, f"Ocurrió un error al cancelar la orden: {str(e)}")
